...
class TestApiUpdateUserData:
    ...
    @allure.description(
        'Вносим изменения во все параметры. Данные авторизованного юзера меняются.'
    )
    ...
    @allure.description(
        'Получение ошибки в ответе метода при попытке внести изменения без авторизации. Данные не меняются.'
    )
